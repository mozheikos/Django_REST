...
from rest_framework.test import APIRequestFactory, force_authenticate, APITestCase, APIClient, CoreAPIClient
...
"""Если я правильно понял, LiveTest выполняется как то так"""
print("Starting Live Test")

test_client = CoreAPIClient()
test_client.session.auth = HTTPBasicAuth("admin", "admin")

endpoint = "http://localhost:8000/api"
links = ("/users/", "/projects/", "/ToDo/")
models = {
    "/users/": User,
    "/projects/": Project,
    "/ToDo/": ToDo
...
for url_link in links:
    test_url = endpoint + url_link
    count = models[url_link].objects.count()

    server_response = test_client.get(test_url)
    assert count == server_response['count']
    print(f"{count=}, {server_response['count']=}")
    print(f"Test for {test_url} passed")
    print("\n", "-" * 50, "\n")
