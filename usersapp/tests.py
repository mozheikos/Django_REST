...
from rest_framework.test import APIRequestFactory, force_authenticate, APITestCase, APIClient, APISimpleTestCase
...
class TestViewsUrls(TestCase):
    ...
    def test_get_pages(self, is_anonymous: bool = True) -> None:
        ...
        for link in self.category_links:
            ...
            print(user, url, response.status_code)
    ...
    def test_get_pages_as_admin(self) -> None:
        ...
        """Напечатал разделитель для удобства"""
        print("\n", "-" * 50, "\n")

        """Вызываю предыдущий тест, но с параметром is_anonymous=False"""
        self.test_get_pages(is_anonymous=False)

    def test_post_delete_method_object(self, is_anonymous: bool = True) -> None:
        client = APIClient()
        user = "anonymous"
        project = None
        if not is_anonymous:
    ...
    def test_post_delete_method_object_as_admin(self) -> None:
        self.test_post_delete_method_object(is_anonymous=False)
